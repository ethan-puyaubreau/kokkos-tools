...
// Stack to manage nested profiling regions (LIFO) //TODO: Mark regions on
// kernel data for plotting
static std::vector<std::string>
    s_profile_regions_stack;                // Use std::vector as a stack
...
struct CFreeDeleter {
  void operator()(char* ptr) const {
    if (ptr) {
      free(ptr);  // Use free() as variorum_get_power_json likely allocates this
                  // way
    }
  }
};
 ...
 * @return unique_json_ptr A smart pointer to the root JSON object, or nullptr
 * on failure.
 */
static unique_json_ptr getVariorumJsonData() {
  ...
  if (variorum_get_power_json(&json_string_c_raw) != 0) {
    // std::cerr << "PowerProfiler: variorum_get_power_json() failed." <<
    // std::endl;
    return nullptr;
  }
  ...
  if (!json_string_c) {
    ...
    // but a null pointer, indicating no data." << std::endl;
    return nullptr;
  }
 ...
 * @return std::map<uint32_t, double> A map of device IDs to their respective
 * power.
 */
std::map<uint32_t, double> getCurrentPowerForDevices(
...
static const int MONITOR_INTERVAL_MS =
    20;  // 20 milliseconds = 50 samples/second : Hardware limitation depending
         // on monitored device
...
void power_monitoring_thread_func(std::stop_token stop_token) {
  while (!stop_token.stop_requested()) {
    std::map<uint32_t, double> current_powers =
        getCurrentPowerForDevices(s_available_gpu_devices);
  ...
  // std::cout << "PowerProfiler: Power monitoring thread stopped.\n";
}
...
void kokkosp_init_library(const int loadSeq, const uint64_t interfaceVer,
  ...
  if (s_available_gpu_devices.empty()) {
    std::cerr << "PowerProfiler: No GPU devices found during initialization. "
                 "Power monitoring will not start.\n";
  } else {
    std::cout << "PowerProfiler: Found " << s_available_gpu_devices.size()
              << " GPU(s):";
    ...
    std::cout << "PowerProfiler: Started background power monitoring thread.\n";
  }
}
...
void kokkosp_finalize_library() {
  if (s_power_monitoring_thread.joinable()) {
    ...
    std::cout << "PowerProfiler: Requested power monitoring thread to stop. "
                 "Waiting for it to finish...\n";
  } else {
    std::cout << "PowerProfiler: Power monitoring thread was not running or "
                 "not joinable.\n";
  }
  ...
  // Detail of the format: Timestamp, Kernel Type, Name, Start Time, End Time,
  // Duration
  std::cout << "\n--- KERNEL_DATA_START ---\n";
  ...
  if (s_kernel_timings_log.empty()) {
    ...
    for (const auto& timing : s_kernel_timings_log) {
      std::cout << "KERNEL_TIMING," << kernel_type_to_string(timing.type)
                ...
                << timing.end_time_ms << ","
                << (timing.end_time_ms - timing.start_time_ms) << '\n';
    }
  }
...
std::string device_type_to_string(
  ...
  switch (deviceType) {
    case Kokkos::Tools::Experimental::DeviceType::Serial: return "SERIAL";
    case Kokkos::Tools::Experimental::DeviceType::OpenMP: return "OPENMP";
    ...
    case Kokkos::Tools::Experimental::DeviceType::Threads: return "THREADS";
    case Kokkos::Tools::Experimental::DeviceType::SYCL: return "SYCL";
    case Kokkos::Tools::Experimental::DeviceType::OpenACC: return "OPENACC";
...
void kokkosp_begin_parallel_for(const char* name, const uint32_t devID,
  ...
  if (kID) {
    ...
    s_active_kernels[*kID] = {std::string(name), KernelType::FOR,
                              current_time_ms, 0};  // Set type to FOR
  }
}
...
void kokkosp_begin_parallel_scan(const char* name, const uint32_t devID,
  ...
  if (kID) {
    std::lock_guard<std::mutex> lock(s_active_kernels_mutex);
    s_active_kernels[*kID] = {std::string(name), KernelType::SCAN,
                              current_time_ms, 0};  // Set type to SCAN
  }
}
...
void kokkosp_begin_parallel_reduce(const char* name, const uint32_t devID,
  ...
  if (kID) {
    std::lock_guard<std::mutex> lock(s_active_kernels_mutex);
    s_active_kernels[*kID] = {std::string(name), KernelType::REDUCE,
                              current_time_ms, 0};  // Set type to REDUCE
  }
}
...
void kokkosp_pop_profile_region() {
  ...
  if (!s_profile_regions_stack.empty()) {
    ...
    printf(
        "KokkosP: Warning: Attempted to pop a profiling region from an empty "
        "stack.\n");
  }
}
