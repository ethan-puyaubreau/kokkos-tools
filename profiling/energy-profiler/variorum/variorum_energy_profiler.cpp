//@HEADER
// ************************************************************************
//
//                        Kokkos v. 4.0
//       Copyright (2022) National Technology & Engineering
//               Solutions of Sandia, LLC (NTESS).
//
// Under the terms of Contract DE-NA0003525 with NTESS,
// the U.S. Government retains certain rights in this software.
//
// Part of Kokkos, under the Apache License v2.0 with LLVM Exceptions.
// See https://kokkos.org/LICENSE for license information.
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
//
//@HEADER

#include "variorum_energy_profiler.hpp"
#include <iostream>
#include <fstream>
#include <set>
#include <cstdlib>
#include <iomanip>
#include <map>

namespace KokkosTools {
namespace PowerProfiler {

VariorumPowerProfiler::VariorumPowerProfiler() {
  if (const char* interval = std::getenv("KOKKOS_TOOLS_POWER_MONITOR_INTERVAL")) {
    try {
      auto interval_us = std::stoul(interval);
      monitor_interval_ = std::chrono::microseconds(interval_us);
    } catch (const std::exception& e) {
      std::cerr << "PowerProfiler: Invalid monitor interval, using default 20ms\n";
    }
  }

  if (const char* output_path = std::getenv("KOKKOS_TOOLS_POWER_OUTPUT_PATH")) {
    output_file_path_ = output_path;
  }
}

VariorumPowerProfiler::~VariorumPowerProfiler() {
  if (initialized_) {
    finalize();
  }
}

bool VariorumPowerProfiler::initialize() {
  if (initialized_) {
    return true;
  }

  if (!initialize_variorum()) {
    std::cerr << "PowerProfiler: Failed to initialize Variorum\n";
    return false;
  }

  available_devices_ = get_available_devices();
  if (available_devices_.empty()) {
    std::cerr << "PowerProfiler: No energy monitoring devices found\n";
    return false;
  }

  // Pre-allocate storage for efficiency
  energy_readings_.reserve(10000);
  completed_kernels_.reserve(1000);
  completed_regions_.reserve(100);

  start_monitoring();
  initialized_ = true;

  std::cout << "PowerProfiler: Initialized with " << available_devices_.size() 
            << " devices, monitoring interval: " << monitor_interval_.count() << "μs\n";

  return true;
}

void VariorumPowerProfiler::finalize() {
  if (!initialized_) {
    return;
  }

  stop_monitoring();
  generate_outputs();
  initialized_ = false;
}

bool VariorumPowerProfiler::initialize_variorum() {
  // Variorum initialization - no specific setup needed for current API
  return true;
}

VariorumPowerProfiler::unique_json_ptr VariorumPowerProfiler::get_variorum_json_data() const {
  char* json_string_c_raw = nullptr;
  int variorum_error = variorum_get_power_json(&json_string_c_raw);

  if (variorum_error != 0) {
    std::cerr << "PowerProfiler: variorum_get_power_json() failed. Error code: "
              << variorum_error << "\n";
    return unique_json_ptr(nullptr);
  }

  unique_cstring json_string_c(json_string_c_raw);

  if (!json_string_c) {
    std::cerr << "PowerProfiler: variorum_get_power_json() returned success "
                 "but a null pointer.\n";
    return unique_json_ptr(nullptr);
  }

  json_error_t error;
  json_t* root_ptr = json_loads(json_string_c.get(), 0, &error);

  if (!root_ptr) {
    std::cerr << "PowerProfiler: Failed to parse JSON: " << error.text << "\n";
    return unique_json_ptr(nullptr);
  }

  return unique_json_ptr(root_ptr);
}

std::vector<uint32_t> VariorumPowerProfiler::get_available_devices() const {
  std::set<uint32_t> found_device_ids;
  unique_json_ptr root = get_variorum_json_data();

  if (!root) {
    return {};
  }

  json_t* host_obj = json_object_iter_value(json_object_iter(root.get()));
  if (!host_obj) {
    return {};
  }

  // Find all GPU devices
  json_t* socket_0 = json_object_get(host_obj, "socket_0");
  if (socket_0 && json_is_object(socket_0)) {
    json_t* power_gpu_watts = json_object_get(socket_0, "power_gpu_watts");
    if (power_gpu_watts && json_is_object(power_gpu_watts)) {
      const char* key;
      json_t* value;
      json_object_foreach(power_gpu_watts, key, value) {
        std::string s_key(key);
        if (s_key.length() > 4 && s_key.substr(0, 4) == "GPU_") {
          try {
            uint32_t device_id = std::stoul(s_key.substr(4));
            found_device_ids.insert(device_id);
          } catch (const std::invalid_argument& e) {
            std::cerr << "PowerProfiler: Could not parse GPU ID from key: " << s_key
                      << " (" << e.what() << ")\n";
          } catch (const std::out_of_range& e) {
            std::cerr << "PowerProfiler: GPU ID out of range from key: " << s_key
                      << " (" << e.what() << ")\n";
          }
        }
      }
    }
  }

  return std::vector<uint32_t>(found_device_ids.begin(), found_device_ids.end());
}

EnergyReading VariorumPowerProfiler::get_current_energy_reading() const {
  EnergyReading reading;
  reading.timestamp = get_current_time();

  unique_json_ptr root = get_variorum_json_data();
  if (!root) {
    return reading;
  }

  json_t* host_obj = json_object_iter_value(json_object_iter(root.get()));
  if (!host_obj) {
    return reading;
  }

  // Only interested in GPU power for all available devices
  json_t* socket_0 = json_object_get(host_obj, "socket_0");
  if (socket_0 && json_is_object(socket_0)) {
    json_t* power_gpu_watts = json_object_get(socket_0, "power_gpu_watts");
    if (power_gpu_watts && json_is_object(power_gpu_watts)) {
      for (uint32_t device_id : available_devices_) {
        std::string gpu_key = "GPU_" + std::to_string(device_id);
        json_t* power_value = json_object_get(power_gpu_watts, gpu_key.c_str());

        if (json_is_number(power_value)) {
          reading.gpu_power_watts[device_id] = json_number_value(power_value);
        }
      }
    }
  }

  return reading;
}

void VariorumPowerProfiler::start_monitoring() {
  monitoring_active_ = true;
  monitoring_thread_ = std::make_unique<std::thread>(&VariorumPowerProfiler::monitoring_thread_function, this);
}

void VariorumPowerProfiler::stop_monitoring() {
  monitoring_active_ = false;
  if (monitoring_thread_ && monitoring_thread_->joinable()) {
    monitoring_thread_->join();
  }
}

void VariorumPowerProfiler::monitoring_thread_function() {
  while (monitoring_active_) {
    EnergyReading reading = get_current_energy_reading();
    
    // Set first measurement time on first reading
    if (!first_measurement_recorded_) {
      first_measurement_time_ = reading.timestamp;
      first_measurement_recorded_ = true;
    }
    
    energy_readings_.push_back(reading);
    std::this_thread::sleep_for(monitor_interval_);
  }
}

void VariorumPowerProfiler::begin_kernel(uint64_t kernel_id, const std::string& name, KernelType type) {
  KernelTiming timing;
  timing.kernel_id = kernel_id;
  timing.name = name;
  timing.type = type;
  timing.start_time = get_current_time();
  
  active_kernels_[kernel_id] = timing;
}

void VariorumPowerProfiler::end_kernel(uint64_t kernel_id) {
  auto it = active_kernels_.find(kernel_id);
  if (it != active_kernels_.end()) {
    it->second.end_time = get_current_time();
    it->second.duration = std::chrono::duration_cast<std::chrono::nanoseconds>(
        it->second.end_time - it->second.start_time);
    
    completed_kernels_.push_back(it->second);
    active_kernels_.erase(it);
  }
}

void VariorumPowerProfiler::push_region(const std::string& name) {
  RegionTiming region;
  region.name = name;
  region.start_time = get_current_time();
  
  active_regions_.push_back(region);
}

void VariorumPowerProfiler::pop_region() {
  if (!active_regions_.empty()) {
    auto& region = active_regions_.back();
    region.end_time = get_current_time();
    region.duration = std::chrono::duration_cast<std::chrono::nanoseconds>(
        region.end_time - region.start_time);
    
    completed_regions_.push_back(region);
    active_regions_.pop_back();
  }
}

std::chrono::time_point<std::chrono::steady_clock> VariorumPowerProfiler::get_current_time() const {
  return std::chrono::steady_clock::now();
}

std::chrono::nanoseconds VariorumPowerProfiler::get_time_relative_to_first_measurement(
    const std::chrono::time_point<std::chrono::steady_clock>& timepoint) const {
  if (!first_measurement_recorded_) {
    return std::chrono::nanoseconds::zero();
  }
  return std::chrono::duration_cast<std::chrono::nanoseconds>(timepoint - first_measurement_time_);
}

void VariorumPowerProfiler::generate_outputs() {
  output_to_console();
  output_to_json();
  output_to_csv();
}

std::string kernel_type_to_string(KernelType type) {
  switch (type) {
    case KernelType::FOR: return "FOR";
    case KernelType::SCAN: return "SCAN";
    case KernelType::REDUCE: return "REDUCE";
    default: return "UNKNOWN";
  }
}

void VariorumPowerProfiler::output_to_console() {
  std::cout << "\n=== Power Profiling Results ===\n";
  std::cout << "Total energy readings: " << energy_readings_.size() << "\n";
  std::cout << "Total kernels executed: " << completed_kernels_.size() << "\n";
  std::cout << "Total regions: " << completed_regions_.size() << "\n";

  if (!energy_readings_.empty()) {
    std::map<uint32_t, double> total_gpu_power;
    for(uint32_t device_id : available_devices_) {
        total_gpu_power[device_id] = 0.0;
    }

    for (const auto& reading : energy_readings_) {
      for (const auto& pair : reading.gpu_power_watts) {
        total_gpu_power[pair.first] += pair.second;
      }
    }
    
    std::cout << std::fixed << std::setprecision(3);
    for (const auto& pair : total_gpu_power) {
      std::cout << "Average GPU " << pair.first << " Power: " << pair.second / energy_readings_.size() << " W\n";
    }
  }

  if (!completed_kernels_.empty()) {
    std::cout << "\nKernel Timings:\n";
    for (const auto& kernel : completed_kernels_) {
      std::cout << "  " << kernel.name << " (" << kernel_type_to_string(kernel.type) 
                << "): " << kernel.duration.count() << " ns\n";
    }
  }
}

void VariorumPowerProfiler::output_to_json() {
  std::ofstream json_file(output_file_path_ + ".json");
  if (!json_file.is_open()) {
    std::cerr << "PowerProfiler: Failed to open JSON output file\n";
    return;
  }

  json_file << "{\n";
  json_file << "  \"power_profiling_results\": {\n";
  json_file << "    \"configuration\": {\n";
  json_file << "      \"monitor_interval_us\": " << monitor_interval_.count() << ",\n";
  json_file << "      \"devices_monitored\": [";
  for (size_t i = 0; i < available_devices_.size(); ++i) {
    json_file << available_devices_[i];
    if (i < available_devices_.size() - 1) json_file << ", ";
  }
  json_file << "]\n";
  json_file << "    },\n";
  
  // Energy readings
  json_file << "    \"power_readings\": [\n";
  for (size_t i = 0; i < energy_readings_.size(); ++i) {
    const auto& reading = energy_readings_[i];
    auto relative_time_ns = get_time_relative_to_first_measurement(reading.timestamp);
    
    json_file << "      {\n";
    json_file << "        \"timestamp_ns\": " << relative_time_ns.count();
    for (const auto& pair : reading.gpu_power_watts) {
        json_file << ",\n        \"GPU_" << pair.first << "_power_watts\": " << pair.second;
    }
    json_file << "\n      }";
    if (i < energy_readings_.size() - 1) json_file << ",";
    json_file << "\n";
  }
  json_file << "    ],\n";
  
  // Kernel timings
  json_file << "    \"kernel_timings\": [\n";
  for (size_t i = 0; i < completed_kernels_.size(); ++i) {
    const auto& kernel = completed_kernels_[i];
    auto start_time_ns = get_time_relative_to_first_measurement(kernel.start_time);
    auto end_time_ns = get_time_relative_to_first_measurement(kernel.end_time);
    
    json_file << "      {\n";
    json_file << "        \"kernel_id\": " << kernel.kernel_id << ",\n";
    json_file << "        \"name\": \"" << kernel.name << "\",\n";
    json_file << "        \"type\": \"" << kernel_type_to_string(kernel.type) << "\",\n";
    json_file << "        \"start_time_ns\": " << start_time_ns.count() << ",\n";
    json_file << "        \"end_time_ns\": " << end_time_ns.count() << ",\n";
    json_file << "        \"duration_ns\": " << kernel.duration.count() << "\n";
    json_file << "      }";
    if (i < completed_kernels_.size() - 1) json_file << ",";
    json_file << "\n";
  }
  json_file << "    ],\n";
  
  // Region timings
  json_file << "    \"region_timings\": [\n";
  for (size_t i = 0; i < completed_regions_.size(); ++i) {
    const auto& region = completed_regions_[i];
    auto start_time_ns = get_time_relative_to_first_measurement(region.start_time);
    auto end_time_ns = get_time_relative_to_first_measurement(region.end_time);
    
    json_file << "      {\n";
    json_file << "        \"name\": \"" << region.name << "\",\n";
    json_file << "        \"start_time_ns\": " << start_time_ns.count() << ",\n";
    json_file << "        \"end_time_ns\": " << end_time_ns.count() << ",\n";
    json_file << "        \"duration_ns\": " << region.duration.count() << "\n";
    json_file << "      }";
    if (i < completed_regions_.size() - 1) json_file << ",";
    json_file << "\n";
  }
  json_file << "    ]\n";
  json_file << "  }\n";
  json_file << "}\n";
  
  json_file.close();
  std::cout << "PowerProfiler: JSON output written to " << output_file_path_ << ".json\n";
}

void VariorumPowerProfiler::output_to_csv() {
  // GPU Power readings CSV
  std::ofstream gpu_power_csv(output_file_path_ + "_gpu_power.csv");
  if (gpu_power_csv.is_open()) {
    gpu_power_csv << "timestamp_nanoseconds,power_watts,device_id\n";
    for (const auto& reading : energy_readings_) {
      auto relative_time_ns = get_time_relative_to_first_measurement(reading.timestamp);
      for (const auto& pair : reading.gpu_power_watts) {
        gpu_power_csv << relative_time_ns.count() << ","
                      << pair.second << ","
                      << pair.first << "\n";
      }
    }
    gpu_power_csv.close();
    std::cout << "PowerProfiler: GPU Power CSV written to " << output_file_path_ << "_gpu_power.csv\n";
  }

  // Kernel timings CSV
  std::ofstream kernels_csv(output_file_path_ + "_kernels.csv");
  if (kernels_csv.is_open()) {
    kernels_csv << "kernel_id,name,type,start_time_ns,end_time_ns,duration_ns\n";
    for (const auto& kernel : completed_kernels_) {
      auto start_time_ns = get_time_relative_to_first_measurement(kernel.start_time);
      auto end_time_ns = get_time_relative_to_first_measurement(kernel.end_time);
      kernels_csv << kernel.kernel_id << ","
                  << "\"" << kernel.name << "\","
                  << kernel_type_to_string(kernel.type) << ","
                  << start_time_ns.count() << ","
                  << end_time_ns.count() << ","
                  << kernel.duration.count() << "\n";
    }
    kernels_csv.close();
    std::cout << "PowerProfiler: Kernels CSV written to " << output_file_path_ << "_kernels.csv\n";
  }

  // Regions CSV
  if (!completed_regions_.empty()) {
    std::ofstream regions_csv(output_file_path_ + "_regions.csv");
    if (regions_csv.is_open()) {
      regions_csv << "name,start_time_ns,end_time_ns,duration_ns\n";
      for (const auto& region : completed_regions_) {
        auto start_time_ns = get_time_relative_to_first_measurement(region.start_time);
        auto end_time_ns = get_time_relative_to_first_measurement(region.end_time);
        regions_csv << "\"" << region.name << "\","
                    << start_time_ns.count() << ","
                    << end_time_ns.count() << ","
                    << region.duration.count() << "\n";
      }
      regions_csv.close();
      std::cout << "PowerProfiler: Regions CSV written to " << output_file_path_ << "_regions.csv\n";
    }
  }
}

} // namespace PowerProfiler
} // namespace KokkosTools